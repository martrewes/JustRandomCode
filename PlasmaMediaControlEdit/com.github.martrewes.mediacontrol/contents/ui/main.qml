...
Item {
    ...
    property string trackLength: {
        ...
        var mprisLength = new Date(xesamLength * 1000).toISOString().substring(14, 19)
        if (xesamLength) {
            return " (" + mprisLength + ")"
        }
    }

    property string trackDate: {

        if (!currentMetadata) {
            return ""
        }
        var xesamDate = currentMetadata["xesam:contentCreated"]
        if (xesamDate) {
            return " [" + xesamDate.slice(0,4) + "]"
        }
    }
