...
class PictureSaver(EventPlugin):
    PLUGIN_ID = "Cover Saver"
    PLUGIN_NAME = _("Cover Saver")
    PLUGIN_DESC = _("MODIFIED: Saves the cover image of the current song to a file.")
    ...
    def plugin_on_song_started(self, song):
        ...
        if song is None:
            ...
            if cover is None:
                ...
                img = Image.open('/home/username/.config/quodlibet/current.cover')
                baseheight = 48
                hpercent = (baseheight / float(img.size[1]))
                wsize = int((float(img.size[0]) * float(hpercent)))
                img = img.resize((wsize, baseheight), Image.ANTIALIAS)
                img.save('/home/martin/.config/quodlibet/cover.jpg')
