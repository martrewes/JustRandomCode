...
std::ostream& operator<<(std::ostream& out, const Network::Bandwidth& speed)
{
    ...
    if (speed.perSecond < MB_i) return out << speed.perSecond / 1000 << " K" << unit;
    return out << std::fixed << std::setprecision(3) << speed.perSecond / MB_f << " M" << unit;
}
...
int main(int argc, char** argv)
{
    ...
    Network::Bandwidth::Unit netSpeedUnit = Network::Bandwidth::Unit::byte;
